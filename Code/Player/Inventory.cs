...
public sealed class Inventory : Component
{
	...
	[Rpc.Broadcast] public void GiveBaseKit()
	{
		if( IsProxy ) return;
		Log.Info( "Dans givebasekit" );
		if(Player.Role == "Murder" )
		{
			//GiveWeapon( knife );
			Log.Info( $"Gave weapon knife to player {Player}" );
		}
		else if (Player.Role == "Cop" )
		{
			Log.Info( $"Gave weapon gun to player {Player}" );
		}
		else if (Player.Role == "Bystander" )
		{
			Log.Info( $"Gave weapon hands to player {Player}" );
		}
	}
	...
	public void DropWeapon( Guid weaponId )
	{
		...
		var tr = Scene.Trace.Ray( new Ray( Player.Head.WorldPosition, Player.Direction.Forward ), 128 )
			.IgnoreGameObjectHierarchy( GameObject.Root )
			.WithoutTags( "trigger" )
			.Run();

		var position = tr.Hit ? (tr.HitPosition + tr.Normal * weapon.Resource.WorldModel.Bounds.Size.Length) : (Player.Head.WorldPosition+ Player.Direction.Forward * 32);
		...
		collider.Center = weapon.Resource.WorldModel.Bounds.Center - (attachObj?.LocalPosition ?? Vector3.Zero);
